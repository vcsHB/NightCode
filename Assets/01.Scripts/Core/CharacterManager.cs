...
namespace Basement.Training
{
    public class CharacterManager : MonoSingleton<CharacterManager>
    {
        ...
        public Vector2 GetCharacterExpectationProfit(CharacterEnum character)
        {
            //나중에 INT 수치에 따라서 수익을 바꿔서 할 수 있게 해주는 느낌으로 바꿔
            int minProfit = 0;
            int maxProfit = 5 + (int)character;

            return new Vector2(minProfit, maxProfit);
        }
